...
def runParse(frac=False,doKin=True):
    '''Parses expression file and UTR sequence file, outputs .ped and .map files for input to PLINK'''
...
def runGEMMA(firstTime=True):
    '''Runs GEMMA using PLINK output files'''
...
def runFAST(phenoFile):
    '''Runs FaST-LMM using PLINK output files'''
    oFile = os.path.basename(plinkf)
...
# parse results
def getResults():
    '''Takes output from GEMMA or FaST-LMM and matches motifs to miRNA seed sequences, and outputs p-values'''
    global resf
